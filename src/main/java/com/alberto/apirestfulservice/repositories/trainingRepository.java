...
public interface trainingRepository extends JpaRepository<training, Long> {
    ...
    @Query(value = "SELECT * FROM training AS i WHERE i.title LIKE %?1% AND i.iduser=?2 LIMIT ?3,10", nativeQuery = true)
    public List<training> getByTitleFromUser(String title, Long code, Long num);

    @Query(value = "SELECT i.id,i.title,i.time,i.published,i.iduser FROM training AS i LEFT JOIN friendship ON i.iduser = friendship.friend_id WHERE i.title LIKE %?1% AND i.published=1 And friendship.owner_id = ?2 LIMIT ?3,10", nativeQuery = true)
    public List<training> getTrainingOfFriendsByTitle(String title, Long code, Long num);

    @Query(value = "SELECT * FROM training AS i WHERE i.iduser=?1 LIMIT ?2,10", nativeQuery = true)
    public List<training> getAllTrainingsByIdUser(Long code,Long num);

    @Query(value = "SELECT * FROM training AS i WHERE i.iduser=?1 AND i.published=1 LIMIT ?2,10", nativeQuery = true)
    public List<training> getAllTrainingsByIdUserIsPublished(Long code,Long num);
    ...
    @Query(value = "SELECT training.id,training.title,training.time,training.published,training.iduser FROM training LEFT JOIN favorites ON training.id = favorites.fk_trainings WHERE favorites.fk_user=?1 LIMIT ?2,10", nativeQuery = true)
    public List<training> getAllTrainingsFromFavorites(Long code,Long num);

    @Query(value = "SELECT training.id,training.title,training.time,training.published,training.iduser FROM training LEFT JOIN favorites ON training.id = favorites.fk_trainings WHERE favorites.fk_user=?1 AND training.title LIKE %?2% LIMIT ?3,10", nativeQuery = true)
