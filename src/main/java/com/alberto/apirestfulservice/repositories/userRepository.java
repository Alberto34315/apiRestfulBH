...
public interface userRepository extends JpaRepository<user, Long> {
     ...
     * Devuelve los usuarios que no son amigos de un usuario
     * SELECT * FROM users LEFT JOIN friendship ON
     * (users.id=friendship.friend_id AND friendship.owner_id = 1) WHERE
     * friendship.friend_id IS NULL AND users.id != 1;
     */
    ...
    @Query(value = "SELECT id,name,email,pass,avatar from users LEFT JOIN friendship ON (users.id=friendship.friend_id AND friendship.owner_id = ?1) WHERE friendship.friend_id IS NULL AND users.id != ?2", nativeQuery = true)
    public List<user> getAllUserLessOwner(Long owner_id1, Long owner_id2);

    @Query(value = "SELECT id,name,email,pass,avatar from users LEFT JOIN friendship ON (users.id=friendship.friend_id AND friendship.owner_id = ?1) WHERE friendship.friend_id IS NULL AND users.id != ?2 AND users.name LIKE %?3%", nativeQuery = true)
    ...
    @Query(value = "SELECT id,name,email,pass,avatar from users LEFT JOIN friendship ON users.id = friendship.friend_id WHERE friendship.owner_id = ?1", nativeQuery = true)
