...
public class userServiceController {
    ...
    @ApiOperation(value = "getAllUsersByIdTrainingFavorite", notes = "Esta funcion nos devolvera una lista de los usuarios, que tienen relacion con un entrenamiento favorito (Pasamos el id del enrenamiento), mas una respuesta HTTP completa")
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "OK. El recurso se obtiene correctamente", response = training.class),
        @ApiResponse(code = 400, message = "Bad Request"),
        @ApiResponse(code = 500, message = "Error inesperado del sistema")})
    @GetMapping("/favoriteUser/{id}")
    public ResponseEntity<List<user>> getAllUsersByIdTrainingFavorite(@PathVariable("id") Long id) throws RecordNotFoundException {
        List<user> entity = service.getAllUsersByIdTrainingFavorite(id);
